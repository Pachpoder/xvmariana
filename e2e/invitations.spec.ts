...
test.skip(
  !canRun,
  'Configura E2E_TESTS=true y las variables E2E_* para usar un proyecto Supabase de prueba aislado.'
);
...
test('crea y edita una invitación', async ({ page }) => {
  ...
  await page
    .getByText('E2E invitación temporal')
    .locator('xpath=ancestor::tr')
    .getByRole('link', { name: 'Editar invitación' })
    .click();
...
test('muestra un enlace inválido y elimina visualmente una invitación', async ({ page }) => {
  ...
  await page
    .getByText('E2E invitación editada')
    .locator('xpath=ancestor::tr')
    .getByRole('button', { name: 'Eliminar invitación del listado' })
    .click();
