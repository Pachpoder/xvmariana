...
test('requiere nombre, apellido y una respuesta válida', () => {
  assert.equal(
    landingRsvpSubmissionSchema.safeParse({ ...validSubmission, firstName: '' }).success,
    false
  );
  assert.equal(
    landingRsvpSubmissionSchema.safeParse({ ...validSubmission, lastName: '' }).success,
    false
  );
  assert.equal(
    landingRsvpSubmissionSchema.safeParse({ ...validSubmission, response: 'pending' }).success,
    false
  );
});
...
test('rechaza el honeypot de la landing', () => {
  assert.equal(
    landingRsvpSubmissionSchema.safeParse({ ...validSubmission, website: 'bot' }).success,
    false
  );
});
