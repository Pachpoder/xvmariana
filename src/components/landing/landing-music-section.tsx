...
export function LandingMusicSection({
  ...
  return (
    <section
      ...
      className='rounded-[1.5rem] border border-[#d8d9c8] bg-[#f5f4ea] px-5 py-5 text-left'
    >
      {musicUrl && (
        <audio
          ref={audioRef}
      ...
      <div className='flex items-center gap-4'>
        <div
          aria-hidden='true'
          className='grid size-11 shrink-0 place-items-center rounded-full bg-[#e7d5d3] text-[#8a6870]'
        >
          <Volume2 size={19} />
        </div>
        <div className='min-w-0 flex-1'>
          <h2 id='music-heading' className='font-serif text-xl text-[#59664d]'>
            Nuestra canción
          </h2>
          <p role='status' className='mt-1 text-xs leading-5 text-[#70796c]'>
            {status}
          </p>
        </div>
        <button
          ...
          aria-label={playback === 'playing' ? 'Pausar música' : 'Reproducir música'}
          className='grid size-11 shrink-0 place-items-center rounded-full bg-[#59664d] text-white transition-colors hover:bg-[#46513c] disabled:cursor-not-allowed disabled:bg-[#aab0a2]'
        >
