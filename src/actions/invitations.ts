...
function parseInvitation(formData: FormData) {
  ...
  let guests: unknown = [];
  try {
    guests = typeof guestsValue === 'string' ? JSON.parse(guestsValue) : [];
  } catch {
    guests = [];
  }
...
async function resolveSlug(input: InvitationInput, invitationId?: string) {
  ...
  const requestedSlug = input.slug
    ...
    : createInvitationSlug(input.guests[0]?.fullName ?? input.label);
...
export async function createInvitation(
  ...
  if (!parsed.success)
    return { error: parsed.error.issues[0]?.message ?? 'Revisa los datos de la invitación.' };
  ...
  const { error } = await supabase.rpc('create_invitation_with_guests', {
    ...
    p_public_slug: slugResult.slug,
    p_max_extra_guests: parsed.data.maxExtraGuests,
    p_internal_notes: parsed.data.internalNotes ?? '',
    p_guests: guestsForRpc(parsed.data),
  });
  if (error)
    return {
...
export async function updateInvitation(
  ...
  if (!parsed.success || !parsed.data.id)
    return {
      error: parsed.success
        ? 'Falta el identificador de la invitación.'
        : (parsed.error.issues[0]?.message ?? 'Revisa los datos de la invitación.'),
    };
  ...
  if (error)
    return {
      error: error.message.includes('already confirmed')
        ? 'No puedes dejar menos nombres que los ya confirmados.'
